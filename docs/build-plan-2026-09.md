...
| 6 | Post effects + Roblox export upgrades: bloom/colour-grade/blur/vignette as Effect Look → three.js post + native Roblox PostEffects; mesh export as .obj + tween script; thin curve meshes → beam chains; volumes → flipbook | done | `render.js` `look` kind + `nodes/render.js` Effect Look (also the sheet's "Bloom & colour grade" thing) + `preview.js` composer from the vendored three r170 passes (RenderPass → UnrealBloomPass → grade/vignette ShaderPass → OutputPass, multisampled half-float target, only while a look is present) + `targets/roblox.js` emitLook (BloomEffect + ColorCorrectionEffect under Lighting, vignette dropped and reported), emitMesh (`objFromGeometry` .obj per source + a MeshPart mover with quaternion CFrames; "Save <name>.obj" in the export dialog) and emitBeam as a chain of ≤8 Beams (`ROBLOX_LIMITS.beamSegments`); volumes were already flipbooks (phase 4). 5 pnxtest checks + 1 smoketest step; 289 PNX, 79/79 smoketest at `9f6666c` |
| 7 | Node editor polish: drag-wire-to-empty-space typed search, auto-layout, minimap, node help panel, keyboard nav | done | `pnx/editorTools.js` (pure: box geometry, layered auto-layout by depth with barycentre rows, nearest-in-direction, `fittingSocket`) + `pnxNodeEditor.js` (a wire released on empty canvas — or a click on a socket — opens the palette filtered to node types with a fitting socket and wires the chosen one in one undo step; Auto-layout button / Ctrl+L / palette command; a minimap canvas with click-drag pan, model rebuilt on change and redrawn once per frame; a help panel from `describeNode` with the Pro badge; arrows / Tab / Enter / Esc keyboard wiring) + `pnx_auto_layout` MCP tool + `pnx_test_editor`; 4 pnxtest checks + 5 smoketest steps; 293 PNX, 84/84 smoketest |
