...
| CON-003 | Scope analysis before a patch | 54 | P0 | implemented (0.12.0) | `ScopeReport` — Part 54's ten rows | `ai/scope.js` | returned by `preview_animation_patch` and `apply_animation_patch` | — | `aitest scope` (9 checks); smoketest asserts a real preview reports a local scope and names the parts that move as a consequence | — | **8 of Part 54's 10 rows are answered from project data.** `expected_visual_region` is null (needs OBS-002/003, Phase 4) and `camera_implications.framing_effect` is null (needs a framing model, Phase 6); both carry the reason and the requirement that unblocks them. Propagation follows the Dependency Graph, so a shoulder edit reaches the wrist, the prop attached to the hand, and any effect on it. The local/broad thresholds are a declared convention, labelled as an `assumption` in the finding's own evidence |
...
| TXN-005 | Restore an approved baseline | 55 | P0 | unplanned | — | — | — | — | — | — | needs REG-001. `restore_snapshot` + a pinned snapshot is the mechanism, and `transaction.decide()` now records acceptance of a TRANSACTION — but nothing marks a SNAPSHOT as an approved baseline, which is what "restore the approved state" requires |
...
| TXN-007 | Fix-only-unintended-changes workflow | 55 | P1 | unplanned | — | — | — | — | — | — | needs REG-001..004 to know WHICH differences were unintended. The narrowing half already exists: a scoped rollback (TXN-003) can reverse exactly one property or frame range once something identifies which |
